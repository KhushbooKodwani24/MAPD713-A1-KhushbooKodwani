...
server.get('/images', function (req, res, next) {
  console.log('< Images GET: received response')
  requestCounterGET++;
...
server.post('/images', function (req, res, next) {
  ...
  if (req.params.imageId === undefined ) {
    ...
    return next(new restify.InvalidArgumentError('Image ID must be provided'))
  }
  if (req.params.name === undefined ) {
    // If there are any errors, pass them to next in the correct format
    return next(new restify.InvalidArgumentError('Image Name must be provided'))
  }
  if (req.params.url === undefined ) {
    // If there are any errors, pass them to next in the correct format
    return next(new restify.InvalidArgumentError('Image URL must be provided'))
  }
  if (req.params.size === undefined ) {
    // If there are any errors, pass them to next in the correct format
    return next(new restify.InvalidArgumentError('Image Size must be provided'))
  }
  var newImage = {
